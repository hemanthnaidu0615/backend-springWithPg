...
@CrossOrigin(origins = "*")
@Tag(name = "Sales Dashboard - Channel", description = "APIs to retrieve order summary data by sales channel")
public class ByChannelController {

    @Autowired
    private PostgresService postgresService;

    @Operation(summary = "Get Order Summary AWW", description = "Fetches filtered and paginated order summary by channel for AWW")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully fetched AWW order summary"),
    ...
    public ResponseEntity<?> getOrderSummaryAww(
        ...
        return fetchOrderSummary("get_order_summary_by_channel_aww", startDate, endDate, page, size, allParams);
    }

    @Operation(summary = "Get Order Summary AWD", description = "Fetches filtered and paginated order summary by channel for AWD")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully fetched AWD order summary"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    @GetMapping("/awd")
    public ResponseEntity<?> getOrderSummaryAwd(
            ...
            @Parameter(description = "Page size for pagination") @RequestParam(defaultValue = "50") int size,
            @Parameter(hidden = true) @RequestParam Map<String, String> allParams) {
        return fetchOrderSummary("get_order_summary_by_channel_awd", startDate, endDate, page, size, allParams);
    }

    private ResponseEntity<?> fetchOrderSummary(String procedureName, String startDate, String endDate, int page, int size, Map<String, String> allParams) {
        try {
            int offset = page * size;

            List<String> allowedFields = List.of("order_type", "total_quantity", "total_amount");
