...
public class ShipmentDashboardKPI {
    ...
    public ResponseEntity<?> getTotalShipments(
        ...
        try {
            ...
            String enterpriseKeyParam = (enterpriseKey == null || enterpriseKey.isBlank()) ? "NULL" : "'" + enterpriseKey + "'";

            String query = String.format("""
                SELECT get_total_shipments('%s'::TIMESTAMP, '%s'::TIMESTAMP, %s::TEXT, %s::TEXT, %s::TEXT) AS total_shipments
            """, startDate, endDate, carrierParam, methodParam, enterpriseKeyParam);
    ...
    public ResponseEntity<?> getOnTimeShipments(
        ...
        try {
            String carrierParam = (carrier == null || carrier.isBlank()) ? "NULL" : "'" + carrier + "'";
            String methodParam = (shippingMethod == null || shippingMethod.isBlank()) ? "NULL" : "'" + shippingMethod + "'";
            String enterpriseKeyParam = (enterpriseKey == null || enterpriseKey.isBlank()) ? "NULL" : "'" + enterpriseKey + "'";

            String query = String.format("""
                SELECT get_on_time_shipments('%s'::TIMESTAMP, '%s'::TIMESTAMP, %s, %s, %s) AS on_time_shipments
            """, startDate, endDate, carrierParam, methodParam, enterpriseKeyParam);
    ...
    public ResponseEntity<?> getDelayedShipments(
        ...
        try {
            ...
            String query = String.format("""
                SELECT get_delayed_shipments('%s'::TIMESTAMP, '%s'::TIMESTAMP, %s, %s, %s) AS delayed_shipments
            """, startDate, endDate, carrierParam, methodParam, enterpriseKeyParam);
    ...
    public ResponseEntity<?> getAverageDeliveryTime(
        ...
        try {
            ...
            String query = String.format("""
                SELECT get_average_delivery_time('%s'::TIMESTAMP, '%s'::TIMESTAMP, %s, %s, %s) AS average_delivery_time
            """, startDate, endDate, carrierParam, methodParam, enterpriseKeyParam);
