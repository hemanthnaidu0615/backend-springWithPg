...
//            @RequestParam(name = "enterpriseKey") String enterpriseKey) {  // Added enterpriseKey as a parameter
//
//        try {
//            // Call the PostgreSQL stored procedure for turnover rate, including the enterprise_key
//        	String turnoverQuery = String.format("SELECT * FROM get_inventory_turnover_and_alerts('%s'::date, '%s'::date, '%s'::text)", startDate, endDate, enterpriseKey);
//  // Pass enterpriseKey to the query
...
//            }
//
//            // Call the PostgreSQL stored procedure for low stock alerts, including the enterprise_key
//            String lowStockQuery = String.format("SELECT * FROM get_low_stock_alerts(%d, '%s')", threshold, enterpriseKey);  // Pass enterpriseKey
